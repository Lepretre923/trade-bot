...
from reports import (
    ...
    institutional, probability, multi_timeframe, market_score,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🤖 Bot trading actif\n\nChoisis une analyse dans le menu.",
        reply_markup=markup
    )

    chat_id = update.effective_chat.id

    context.job_queue.run_repeating(market_alert, interval=300, first=60, chat_id=chat_id)
    context.job_queue.run_repeating(auto_market_report, interval=7200, first=60, chat_id=chat_id)

    context.job_queue.run_daily(daily_trade_plan, time=datetime.time(hour=7, minute=30), chat_id=chat_id)

    context.job_queue.run_daily(daily_report, time=datetime.time(hour=8, minute=0), chat_id=chat_id)
    context.job_queue.run_daily(daily_report, time=datetime.time(hour=13, minute=0), chat_id=chat_id)
    context.job_queue.run_daily(daily_report, time=datetime.time(hour=20, minute=0), chat_id=chat_id)

    context.job_queue.run_repeating(danger_alert, interval=300, first=60, chat_id=chat_id)
    context.job_queue.run_repeating(opportunity_alert, interval=600, first=60, chat_id=chat_id)
    context.job_queue.run_repeating(breakout_alert, interval=900, first=60, chat_id=chat_id)
...
async def message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if t == "💰 BTC Analyse":
        ...
        await update.message.reply_text(fear_greed())

    elif t == "📉 Funding rate":
        await update.message.reply_text(funding_rate())

    elif t == "📊 Open interest":
        await update.message.reply_text(open_interest())

    elif t == "🔥 Liquidation map":
        await update.message.reply_text(liquidation_map())

    elif t == "📊 Volume profile":
        await update.message.reply_text(volume_profile())

    elif t == "💵 Stablecoin flow":
        await update.message.reply_text(stablecoin_flow())

    elif t == "🌍 Marché global":
        await update.message.reply_text(global_market())
